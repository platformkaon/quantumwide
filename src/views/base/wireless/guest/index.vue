<template>
	<section class="section">
		<div class="module-tab">
			...
			<form :name='guestWifiForm.name' @submit.prevent='save()' v-loading="guestWifiForm.loading">
									...
									<b-card-text>
										<tbody>
											...
											<div class="form-checkbox">
												<input type="checkbox" id="guest_5G_enable" name='guest_5G_enable' class="checkbox" v-model='guestWifiForm.fields.disabled' true-value='0' false-value='1'>
											...
											<tr>
												...
												<td>
													<div class="form-checkbox">
														<input type="checkbox" id="guest_5G_hidden" name='guest_5G_hidden' class="checkbox" v-model='guestWifiForm.fields.hidden' true-value='1' false-value='0'>
											...
											<tr>
												...
												<td>
													<input type="text" class="form-control" name='guest_5G_ssid' id="guest_5G_ssid" v-model='guestWifiForm.fields.ssid'>
												</td>
											</tr>

											<tr v-if="guestWifiForm.fields.encryption !== 'none'">
												<th >
												...
												<td>
													<div class="form-password-wrap" :class="{'type-view': !showPassword}">
														<input :type='showPassword ? "text" : "password"' class="form-control" id="guest_5G_password" placeholder="" v-model='guestWifiForm.fields.key'>
											...
											<tr>
												...
												<td>
													<select class="form-select" v-model='guestWifiForm.fields.encryption' name='guest_5G_encryption'>
									...
									<b-card-text>
										<tbody>
											<tr>
												...
												<td>
													<div class="form-checkbox">
														<input type="checkbox" id="guest_2G_enable" name='guest_2G_enable' class="checkbox" v-model='guestWifiForm.fields.disabled' true-value='0' false-value='1'>
											...
											<tr>
												...
												<td>
													<div class="form-checkbox">
														<input type="checkbox" id="guest_2G_hidden" name='guest_2G_hidden' class="checkbox" v-model='guestWifiForm.fields.hidden' true-value='1' false-value='0'>
											...
											<tr>
												...
												<td>
													<input type="text" class="form-control" name='guest_2G_ssid' id="guest_2G_ssid" v-model='guestWifiForm.fields.ssid'>
												</td>
											</tr>
											...
											<tr v-if="guestWifiForm.fields.encryption !== 'none'">
												...
												<td>
													<div class="form-password-wrap" :class="{'type-view': !showPassword}">
														<input :type='showPassword ? "text" : "password"' class="form-control" id="guest_2G_password" placeholder="" v-model='guestWifiForm.fields.key'>
														<span class="icon-view-passowrd" @click="togglePassword('new')"></span>
													</div>
												</td>
											</tr>
											<tr>
												...
												<td>
													<select class="form-select" v-model='guestWifiForm.fields.encryption' name='guest_2G_encryption'>
														<option v-for='item in encryptionOptions'
															:value='item.value'
															:key='item.value'>
															{{ item.label }}
									...
									<b-card-text>
										<tbody>
											<tr>
												...
												<td>
													<div class="form-checkbox">
														<input type="checkbox" id="guest_6g_enable" name='guest_6g_enable' class="checkbox" v-model='guestWifiForm.fields.disabled' true-value='0' false-value='1'>
											...
											<tr>
												...
												<td>
													<div class="form-checkbox">
														<input type="checkbox" id="guest_6g_hidden" name='guest_6g_hidden' class="checkbox" v-model='guestWifiForm.fields.hidden' true-value='1' false-value='0'>
											...
											<tr>
												...
												<td>
													<input type="text" class="form-control" name='guest_6g_ssid' id="guest_6g_ssid" v-model='guestWifiForm.fields.ssid'>
												</td>
											</tr>
											...
											<tr v-if="guestWifiForm.fields.encryption !== 'none'">
												...
												<td>
													<div class="form-password-wrap" :class="{'type-view': !showPassword}">
														<input :type='showPassword ? "text" : "password"' class="form-control" id="guest_6g_password" placeholder="" v-model='guestWifiForm.fields.key'>
														<span class="icon-view-passowrd" @click="togglePassword('new')"></span>
													</div>
												</td>
											</tr>
											<tr>
												...
												<td>
													<select class="form-select" v-model='guestWifiForm.fields.encryption' name='guest_6g_encryption'>
														<option v-for='item in encryptionOptions'
															:value='item.value'
															:key='item.value'>
															{{ item.label }}
...
export default {
	...
	watch: {
		'currentTab' (radio) {
			if (radio) {
				this.guestWifiForm.loading = true
				this.fetchData()
			}
		}
	},
	methods: {
		...
		loadWiFi () {
			ws.call(API.wireless.guest.get).then(({ dict }) => {
				dict.list.forEach(item => {
					if (item.band === "5g") {
						this.tabs.push({ id: item.name, type: '5g', label: 'guest', label_2: '5ghz' })
						return false
					}
				})
				dict.list.forEach(item => {
					if (item.band === "2g") {
						this.tabs.push({ id: item.name, type: '2g', label: 'guest', label_2: '2.4ghz' })
						return false
					}
				})
				dict.list.forEach(item => {
					if (item.band === "6g") {
						this.tabs.push({ id: item.name, type: '6g', label: 'guest', label_2: '6ghz' })
						return false
					}
				})
			})
		...
		fetchData() {
			...
			this.guestWifiForm.loading = true
			requests[1] = ws.call(apiRPC("/wifi/ssid/guest/" + this.currentType, "get", {})).then(({ dict: ssidResp }) => {
				if (ssidResp.list[0]) {
					fillObject(this.guestWifiForm.fields, ssidResp.list[0])

					if (this.guestWifiForm.fields.encryption === 'none') {
						qr.addData(`WIFI:T:nopass;S:${this.guestWifiForm.fields.ssid};;;`)
					}
					else {
						qr.addData(`WIFI:T:WPA;S:${this.guestWifiForm.fields.ssid};P:${this.guestWifiForm.fields.key};;`)
					}
					qr.make()
			...
			Promise.all(requests).catch(reportError).finally(() => {
				this.loading = false
				this.guestWifiForm.loading = false
			})
		},
		async save () {
			QSpinshow()
			...
			if(str_length(this.guestWifiForm.fields.ssid, 1, 32)){
				reportError(this.t('err.validation.wifi-ssid-1').cap)
				return QSpinhide()
			}
			else if(blank_pattern.test(this.guestWifiForm.fields.ssid)==true){
				reportError(this.t('ssid-space-alert').cap)
				return QSpinhide()
			}
			else if(str_length(this.guestWifiForm.fields.key, 8, 32) && this.guestWifiForm.fields.encryption !== 'none'){
				reportError(this.t('err.validation.wifi-pass-1').cap)
				return QSpinhide()
			}
			else if(str_length(this.guestWifiForm.fields.key, 8, 63) && this.guestWifiForm.fields.encryption !== 'none'){
				reportError(this.t('err.validation.wifi-pass-7').cap)
				return QSpinhide()
			}
			else if(blank_pattern.test(this.guestWifiForm.fields.key)==true){
				reportError(this.t('password-blank-no').cap)
				return QSpinhide()
			}
			else if(!validator.isAscii(this.guestWifiForm.fields.key)||!validator.isAscii(this.guestWifiForm.fields.ssid)) {
				reportError(this.t('ssid').cap+", "+this.t('password').cap+" : "+this.t('err.validation.wifi-pass-6').cap)
				return QSpinhide()
			}

			requests.push(
				ws.call(API.wireless.ssid.set, this.guestWifiForm.fields)
			)
